...
async def generate(
    ...
    try:
        checked = Image.open(io.BytesIO(raw))
        checked.verify()
    ...
    full_prompt = (
        ...
        "Photorealistic real-camera appearance, natural skin texture, realistic lighting, "
        "correct anatomy, believable perspective and depth. "
        "The same person must remain recognizable."
    )

    try:
        # Send the reference image directly as a Base64 data URI.
        # This avoids fal storage upload/auth and the 403 /storage/auth/token error.
        mime = face_image.content_type or "image/jpeg"
        encoded = base64.b64encode(raw).decode("ascii")
        reference_data_url = f"data:{mime};base64,{encoded}"

        arguments = {
            "model_type": "1_5-v1",
            "prompt": full_prompt,
            "face_image_url": reference_data_url,
            "negative_prompt": (
